...
void Interpreter::zero_token_is_type_parse(std::vector<std::string>& tokens) {
    ...
    } else if (tokens.size() == 4 && tokens[2] == "=" && !is_declared_variable(tokens[3]) && !is_declared_array(separate_name_and_size_in_array_declaration(tokens[3]).first)) { // case if int x = 5; like declaration
    ...
    } else if (tokens.size() == 4 && tokens[2] == "=" && is_declared_array(separate_name_and_size_in_array_declaration(tokens[3]).first)) {
        parse_value_in_array_to_var(tokens[0], tokens[1], tokens[3]);
