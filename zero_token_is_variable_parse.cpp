...
void Interpreter::first_token_is_variable_parse(std::vector<std::string>& tokens) {
    if (tokens.size() == 3 && is_declared_variable(tokens[0]) && tokens[1] == "=" && !is_declared_array(separate_name_and_size_in_array_declaration(tokens[2]).first)) {
    ...
    } else if (tokens.size() == 5 && is_declared_variable(tokens[0]) && tokens[1] == "=") { // x = y + z
    ...
    } else if (tokens.size() == 3 && is_declared_variable(tokens[0]) && tokens[1] == "=" && is_declared_array(separate_name_and_size_in_array_declaration(tokens[2]).first)) { // case k = arr[3]
        parse_value_in_array_to_var(type_of_var(tokens[0]), tokens[0], tokens[2]);
